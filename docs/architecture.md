...
2. ~~固件无持久化~~（**已修复** 2026-08-30：步数 + 时间偏移用 NVS/Preferences 持久化；设置、RTC 精确时间待做）。
3. 固件**无低功耗/深睡**，续航未优化。
4. App 的 `NotificationService` 仅初始化，**未接入**跌倒/心率/血氧告警通知。
5. App **「记住设备 + 自动重连」未生效**（`device_page` 未调用 `last_device_id`）。
6. App `BleService.get rssi` 为死代码（恒返回 -100）。
